...
contract Scarf is ERC721Enumerable {
  ...
  mapping(uint256 => uint256) public crow_scarf;

  //! Properties types
  string[3] public colors;

  constructor() ERC721("CrowScarf", "CSF") {
  ...
  function mintItem() public payable returns (uint256) {
    ...
    crow_scarf[id] = uint256((uint8(predictableRandom[8])) % 3);
  ...
  function renderTokenById(uint256 id) public view returns (string memory) {
    ...
    string memory render = string(abi.encodePacked(ScarfLibrary.getScarf(colors[color])));
  ...
  function getDescription(uint256 id) public view returns (string memory) {
    ...
    uint256 scarf = getPropertiesById(id);
    string memory desc = string(abi.encodePacked("Color: ", colors[scarf]));
    return desc;
  }
  ...
  function tokenURI(uint256 id) public view override returns (string memory) {
    ...
    string memory name = string(abi.encodePacked("Scarecrow Scarf #", id.toString()));

    string memory description = string(abi.encodePacked("Color: ", colors[scarf]));
          ...
          Base64.encode(
            bytes(
              abi.encodePacked(
                ...
                id.toString(),
                '","attributes":[{"trait_type":"ScarfColor","value":"',
                colors[scarf],
                '"}], "owner":"',
  ...
  function getPropertiesById(uint256 id) public view returns (uint256 scarf) {
    scarf = crow_scarf[id];
  }
}
